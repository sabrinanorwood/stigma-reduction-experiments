...
SD items are only reverse scored for pre and post, not followup, so sum scores at that timepoints is meaningless. correct.

# Notes

Both MHS and MASS have been scored for this study so that higher scores are desirable, i.e., represent lower homophobia. This is so that (a) there is a congruent interpretation of citonality in the analyses, and (b) so that positive Cohen's d scores in the meta analyses represent improvement, which is typical. 
...
data_item_level_prepost_a <- read_excel("raw/data_experiment_4_raw_prepost_a.xlsx") %>%
  ...
  rename(id                         = q62,
         age                        = q3,
         gender                     = q4,
         gender_text                = q4_3_text,
         sexual_orientation         = q5,
         sexual_orientation_text    = q5_5_text,
         ethnicity                  = q6,
         ethnicity_text             = q6_8_text,
         political_identity         = q7,
         attentioncheck1_condition1 = q42,
         attentioncheck2_condition1 = q43,
         attentioncheck1_condition2 = q44,
         attentioncheck2_condition2 = q45,
         attentioncheck1_condition3 = q25,
         attentioncheck2_condition3 = q26,
         attentioncheck1_condition4 = q47,
         attentioncheck2_condition4 = q48,
         attentioncheck1_condition5 = q49,
         attentioncheck2_condition5 = q50,
         attentioncheck1_condition6 = q51,
         attentioncheck2_condition6 = q52,
         attentioncheck3_condition3 = q27,
         attentioncheck3_condition6 = q53,
         # rename to extract the socially desirable responding questions
         sd_pre_1   = massa_1, # these differ between conditions
         sd_pre_2   = massa_3,
         sd_pre_3   = massa_4,
         sd_pre_4   = massa_6,
         sd_pre_5   = massa_7,
         ...
         mass_pre_1   = massa_2, # these differ between conditions
         mass_pre_2   = massa_5,
         mass_pre_3   = massa_10,
         mass_pre_4   = massa_14,
         mass_pre_5   = massa_16,
         ...
         agree_with_marriage_equality = q56_1,
         business_right_to_refuse_sexuality = q57_1,
         willingness_to_sign_petition = q58,
         
         long_format_response_1  = q16, 
         long_format_response_2  = q18, 
         long_format_response_3  = q20, 
         long_format_response_4  = q22, 
         long_format_response_5  = q24, 
         long_format_response_6  = q33, 
         long_format_response_7  = q35, 
         long_format_response_8  = q37, 
         long_format_response_9  = q39, 
         long_format_response_10 = q41) %>%
  ...
  # rename all variables that match a criterion
  rename_with(~ str_replace(., "q16_",   "iri_"),        .cols = starts_with("q16_")) %>%
  rename_with(~ str_replace(., "q17_",   "toe_"),        .cols = starts_with("q17_")) %>%
  rename_with(~ str_replace(., "mhs_g_", "mhs_g_pre_"),  .cols = starts_with("mhs_g_")) %>%
  rename_with(~ str_replace(., "mhs_l_", "mhs_l_pre_"),  .cols = starts_with("mhs_l_")) %>%
  rename_with(~ str_replace(., "q54_",   "mhs_g_post_"), .cols = starts_with("q54_")) %>%
  rename_with(~ str_replace(., "q55_",   "mhs_l_post_"), .cols = starts_with("q55_"))  %>%
  ...
  mutate(mhs_g_pre_1   = ifelse(!is.na(mhs_g_pre_1),   8 - mhs_g_pre_1,   NA),
         ...
         sd_pre_1      = ifelse(!is.na(sd_pre_1),      8 - sd_pre_1,      NA),
  ...
  mutate(condition = case_when(!is.na(attentioncheck1_condition1) | !is.na(attentioncheck2_condition1) ~
                                 "traditional__read",
                               !is.na(attentioncheck1_condition2) | !is.na(attentioncheck2_condition2) ~
                                 "traditional__read_plus_instruction_to_PT",
                               !is.na(attentioncheck1_condition3) | 
                                 !is.na(attentioncheck2_condition3) |
                                 !is.na(attentioncheck3_condition3) ~ 
                                 "traditional__read_plus_rewrite_from_1st_person",
                               !is.na(attentioncheck1_condition4) | !is.na(attentioncheck2_condition4) ~ 
                                 "modelled__read",
                               !is.na(attentioncheck1_condition5) | !is.na(attentioncheck2_condition5) ~ 
                                 "modelled__read_plus_instruction_to_PT",
                               !is.na(attentioncheck1_condition6) | 
                                 !is.na(attentioncheck2_condition6) |
                                 !is.na(attentioncheck3_condition6) ~ 
                                 "modelled__read_plus_rewrite_from_1st_person")) %>%
  separate(condition, into = c("condition_type_of_pt", "condition_level"), sep = "__", remove = FALSE) %>%
  # wrangle attention checks
  mutate(attention_check_1 = case_when(!is.na(attentioncheck1_condition1) ~ attentioncheck1_condition1,
                                       !is.na(attentioncheck1_condition2) ~ attentioncheck1_condition2,
                                       !is.na(attentioncheck1_condition3) ~ attentioncheck1_condition3,
                                       !is.na(attentioncheck1_condition4) ~ attentioncheck1_condition4,
                                       !is.na(attentioncheck1_condition5) ~ attentioncheck1_condition5,
                                       !is.na(attentioncheck1_condition6) ~ attentioncheck1_condition6),
         attention_check_2 = case_when(!is.na(attentioncheck2_condition1) ~ attentioncheck2_condition1,
                                       !is.na(attentioncheck2_condition2) ~ attentioncheck2_condition2,
                                       !is.na(attentioncheck2_condition3) ~ attentioncheck2_condition3,
                                       !is.na(attentioncheck2_condition4) ~ attentioncheck2_condition4,
                                       !is.na(attentioncheck2_condition5) ~ attentioncheck2_condition5,
                                       !is.na(attentioncheck2_condition6) ~ attentioncheck2_condition6),
         attention_check_3 = case_when(!is.na(attentioncheck3_condition3) ~ attentioncheck3_condition3,
                                       !is.na(attentioncheck3_condition6) ~ attentioncheck3_condition6),
         attention_check_1_scored = ifelse(str_detect(tolower(attention_check_1), "richard"), "Correct", "TO BE MANUALLY SCORED"),
         attention_check_2_scored = ifelse(str_detect(tolower(attention_check_2), "27"), "Correct", "TO BE MANUALLY SCORED")) %>%
  # handle duplicates
...
data_item_level_prepost_b <- read_excel("raw/data_experiment_4_raw_prepost_b.xlsx") %>%
  # automatically clean variable names
  janitor::clean_names() %>%
  ...
  rename(id                         = q62,
         ...
         attentioncheck3_condition6 = q53,
         # rename to extract the socially desirable responding questions
         sd_post_1  = massa_1, # these differ between conditions
         sd_post_2  = massa_3,
         sd_post_3  = massa_4,
         sd_post_4  = massa_6,
         sd_post_5  = massa_7,
         ...
         mass_pre_1 = massb_1,  # these differ between conditions
         mass_pre_2 = massb_4,
         mass_pre_3 = massb_5,
         mass_pre_4 = massb_7,
         mass_pre_5 = massb_11,
...
data_item_level_prepost <- 
  ...
  mutate(gender = ifelse(gender == "Or, if you prefer, type your answer", gender_text, gender),
         sexual_orientation = ifelse(sexual_orientation == "Or, if you prefer, type your answer", sexual_orientation_text, sexual_orientation),
         ethnicity = ifelse(ethnicity == "Or, if you prefer, type your answer", ethnicity_text, ethnicity)) %>%
  select(-gender_text, -sexual_orientation_text, -ethnicity_text)
...
data_item_level_followup <- read_excel("raw/data_experiment_4_raw_followup.xlsx") %>%
...
data_processed <- data_processed_for_hand_scoring %>% 
  select(-attention_check_1_scored,
         -attention_check_2_scored) %>%
